...
class CommunitiesSerializer(serializers.ModelSerializer):
    last_message = serializers.CharField(read_only = True)
    unread_chat = serializers.CharField(read_only = True)
    last_message_sender = serializers.IntegerField(read_only = True) 
    
    creator = MiniUserSerializer(User)
    class Meta :
        model = Community
        fields =["id","name","date",'creator','members','picture',"last_message","unread_chat","last_message_sender"]
...
class FriendChatsSerializer(serializers.ModelSerializer):
    ...
    blocked_users = MiniUserSerializer(User,many=True)
    class Meta :
        model = User
        fields = ['id',"username",'last_name','picture','first_name','chats','profile','blocked_users']
