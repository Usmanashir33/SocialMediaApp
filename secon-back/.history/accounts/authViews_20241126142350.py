    ...
    def post(self, request,format=None):
        # try :
            ...
            if found :
                ...
                # user.save()
            return Response({"email_sent" :f"{email}" } if found else {"error" : "No account associated with this email"}) # either true or false
        # except :
...
class CheckFPWOTPView(APIView):
    ...
    def post(self, request,format=None):
        try :
            ...
            user = User.objects.get(email = email).forget_password_otp == otp
            return Response({"verified" : user} if user else {"error" : "password not correct"}) # either true or false
