...
class ForgetPasswordSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
            ...
            message = f"""
                <div>
                    <h2>Your Account Password is Reset</h2>
                    <p>if you did not perform the action kindly reply to this email or change your password</p>
                </div>"""
            ...
            mail.send()
            instance.save()
            updated = instance.check_password(validated_data['password'])
            return ({'updated':True} if updated else {'error':"password not updated"})