    ...
    def put(self, request, user_id,format=None):
        ...
        try :
            if request.user.id == user_id:
                user = User.objects.get(id = user_id) # we get user intance
                profile = user.profile # we get profile instace
                Nfile = file if file != "null" else None
                currentFile = Nfile == f"/media/{user.picture}"
                
                if user :
                    user.first_name =data.get('first_name')
                    user.last_name =data.get('last_name')
                    if not currentFile : #only if the file field is altered
                        user.picture = Nfile 
...
class UserMediaView(APIView):
    def get(self, request, user_id,format=None):
        try :
            posts = Post.objects.filter(Q(user = user_id)).exclude((Q(file__isnull=True) | Q(file='')).order_by("-date")
            if posts :
                serializer = PostSerializer(posts ,many=True)
                return Response(serializer.data)
            else :
...
class UserFollow(APIView):
    def get(self, request, user_id,format=None):
        try:
            ...
            if user.id != target_user.id : #user can not follow him self
                if user in target_user.followers.all(): #user already in followers ,remove him
                    ...
                    notification.members.add(target_user)
                    return Response({"follow":"followed"}) if notification else  Response({"follow":"failed"}) 
...
class UserBlock(APIView):
    def get(self, request, user_id ,format=None):
        try:
            ...
            if friend in user.blocked_users.all() :
                user.blocked_users.remove(friend)
                inform_user_blocked(friend.id,user.id,"unblocked") # to let friend know hes blocked 
                return Response({'unblocked':"unblocked"})
            else:
                user.blocked_users.add(friend)
                inform_user_blocked(friend.id,user.id,"blocked") # to let friend know hes blocked 
...
def inform_user_blocked(friend_id,user_id,action):
    ...
    async_to_sync(channel_layer.group_send)(
        # f"chat_room{friend_id}", info9
        f"chat_room{friend_id}", info
    )