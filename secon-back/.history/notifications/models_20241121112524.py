...
class CommunityDelNotification(models.Model):
    status = models.CharField(_("status"), max_length=50,default="community_deletion")
...
class FollowingNotification(models.Model):
    status = models.CharField(_("status"), max_length=50,default="following")
    follower = models.ForeignKey(User, verbose_name=_("follower"), related_name='whofollow',on_delete=models.DO_NOTHING,blank=True)
    following = models.ForeignKey(User, verbose_name=_("following"),related_name='', on_delete=models.DO_NOTHING,blank=True)
