...
class UserNotificationView(APIView): 
    def get(self, request):
        try:
            ...
            follow_notifications = user.whofollow.all().filter(members=user)
...
class UserReadNotificationView(APIView):
    def get(self, request):
        ...
        try:
            user = request.user
            com_notifications = user.comdelnot.all()
            fol_notifications = user.followingNotif.all()
...
class UserDeleteAllNotificationView(APIView):
    def delete(self, request,notification_i):
        try:
            user = request.user
            com_notifications = user.comdelnot.all()
            fol_notifications = user.followingNotif.all()
