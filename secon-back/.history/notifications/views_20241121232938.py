...
class UserReadNotificationView(APIView):
    def get(self, request):
        ...
        # try:
            user = request.user
            com_notifications = user.comdelnot.all()
            fol_notifications = user.readFNotif.all()
            # total_notif = list(com_notifications) + list(fol_notifications)
            for notif in fol_notifications :
...
class UserDeleteFNotificationView(APIView):
    def delete(self, request,notification_id):
        # try:
            ...
            # fol_notification.readers.remove(user) 
