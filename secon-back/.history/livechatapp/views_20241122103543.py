...
class AllChats(APIView):
    def get(self, request, format=None):
        try:
            ...
            unread_chats =  Message.objects.filter(Q(user_to=user.id,user_from = OuterRef("id"),read=False))
            ...
            chatfriends = User.objects.filter(
                ...
                unread_chats = Count(Subquery(unread_chats.values_list("id"))),
...
class DeleteMessageForMe(APIView):
    def delete(self, request,message_id): 
        try:
            user = request.user
            chat = Message.objects.get(id = message_id)
            chat.delete_for_me.add(user)
            return Response({'deleted':"deleted"}) 
        except :
            return Response({"error":"error delete for me"}) 
class GetUserMessages(APIView):
    def get(self, request, user_id, format=None): 
        try:
            ...
            chats = Message.objects.filter(
                Q(user_from=user.id,user_to = user_id) |
                Q(user_to = user.id,user_from = user_id) & Q(delete_for_me = False)
            ).order_by("date")
            serializer =  FriendChatsSerializer(friend, context={"chats" : chats,"profile" :profile}) 
            return Response(serializer.data) 
        except :
    ...
    def get(self, request, format=None):
        ...
        try :
            if user :
                ...
                unread_chats = CommunityChat.objects.filter(chat_community = OuterRef("id")).exclude(readers__id = user.id).exclude(user_from = user.id)
                communities = Community.objects.filter(members=user).annotate(
                    ...
                    unread_chat = Count(Subquery(unread_chats.values("id"))),
                ).order_by(
                    "-last_message_date"
                )
