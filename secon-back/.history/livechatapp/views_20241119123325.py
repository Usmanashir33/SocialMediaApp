    ...
    def get(self, request, format=None):
        ...
        try :
            if user :
                ...
                unread_chats = CommunityChat.objects.filter(chat_community = OuterRef("id")).exclude(readers__id = user.id)
                communities = Community.objects.filter(members=user).annotate(
                    last_message = Subquery(community_chats.values("message")[:1]),
                    unread_chat = Count(Subquery(unread_chats.values("message"))),
