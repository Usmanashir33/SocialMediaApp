...
class AllChats(APIView):
    def get(self, request, format=None):
        try:
            ...
            unread_chats = Message.objects.filter(Q(user_from = OuterRef("id"),user_to = user.id ,read=False ))
            ...
            chatfriends = User.objects.filter(
                ...
                msg = Subquery(unread_chats.values('id'))
            ).order_by(
                "-last_message_date"
            )
            serializer =  ChatFriendSerializer(chatfriends,many=True) 
            print(FIELDNAME = serializers.ReadOnlyField())
