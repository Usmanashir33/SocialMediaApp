...
class AllChats(APIView):
    def get(self, request, format=None):
        try:
            ...
            unread_chats =  Message.objects.filter(Q(user_from=user.id,user_to = OuterRef("id")) |
                                           Q(user_to = user.id,user_from = OuterRef("id")))
            ...
            chatfriends = User.objects.filter(
                ...
                unread_chats = Subquery(unread_chats.values_list("id",flat=True).filter(read=False).count),
