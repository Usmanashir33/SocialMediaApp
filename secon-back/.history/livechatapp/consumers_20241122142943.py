    ...
    async def receive(self, bytes_data=None, text_data=None):
        ...
        if text_data :
            ...
            if data['status'] == 'message' and not data['withFile']: # the file is not sent
                ...
                # friend = await get_user_by_id(self.user_to)
                blocked = self.user.blocked_users.filter(id = self.user_to)
                print('blocked: ', blocked)
                # if friend and friend not in self.user.blocked_users.all() :
                if blocked :
                    print("hi")
            ...
            if data['status'] == "unreadMessages" :
                self.user_from =data['user_from'] # imean frd here its mis use 
                self.friend_rm = f"chat_room{self.user_from}"
                messageDetails ={
                ...
                if message == 'done':
                    ...
                    await self.channel_layer.group_send(
                        self.friend_rm , message
                        )
            if data['status'] == "deleteMessageForAll" :
...
@database_sync_to_async    
def check_if_blocked(user,friend_id) :
        friend = User.objects.get(id = friend_id)
        blocked = friend in user.blocked_users.all()
        
        return True if blocked else Fals
...
def readMessages(data):
    ...
    friend_id = data['user_from']
    messages = Message.objects.filter(
       Q(user_from = friend_id,user_to = user_id) & Q(read = False)) 
    for message in messages :
       message.read = True
       message.save()
    return "done" if messages else None

@database_sync_to_async
def deleteMessageForAll(data):
    user_id = data['user_id']
    friend_id = data['friend_id']
    message_id = data['message_id']
    
    message = Message.objects.get(id = message_id)
    if message and message.user_from.id == user_id :
        message.message='message deleted'
        # message.date=None
        if message.media : 
            message.media = None
        message.save()
        delSerializer = MessageSerializer(message).data
        # print('delSerializer: ', delSerializer)
