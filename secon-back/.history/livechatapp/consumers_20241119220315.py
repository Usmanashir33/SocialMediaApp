...
class CommunityChatConcumer(AsyncWebsocketConsumer) :
    ...
    async def receive(self, bytes_data=None, text_data=None):
        ...
        if text_data :
            ...
            if data['status'] == "unreadChats" :
                self.chat_id = data['chat_id'] # imean frd the particular community 
                messageDetails ={
                    "user_id" : self.user_id,
                    'chat_id' : self.chat_id
                }
                message = await readChats(messageDetails)
                if message == 'done':
                    message = {
                        'type':'chat_message',
                        'status' :'unreadChat',
...
def readChats(data):
    ...
    chats = CommunityChat.objects.filter(
       Q(user_from = friend_id,user_to = user_id) & Q(read = False)) 
    for message in messages :
       message.read = True
       message.save()
