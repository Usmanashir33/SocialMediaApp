        ...
        try:
            ...
            if post.user.id == request.user.id and not_blank:
                ...
                if serializer.is_valid():
                    serializer.save()
                    if not file : # removing the file from the media folder as well
    ...
    def delete(self, request, id, format=None):
        try:
            ...
            if post.user.id == user.id :
                os.remove(post.file.path) if post.file else None # to remove the file from the media folder as well
                post.delete()
    ...
    def put(self, request, comment_id,format=None):
        ...
        # try:
            ...
            if comment.user.id == user_id and not_blank :
                os.remove(comment.file.path) if comment.file and file else None
    ...
    def delete(self, request, comment_id,format=None):
        try:
            ...
            if comment.user.id == user_id : 
                os.remove(comment.file.path) if comment.file else None
                comment.delete()
                return Response({"success":'deleted '})
            else:
