...
class PostsView(APIView):
    ...
    def get(self, request, format=None, *args, **kwargs):
        try:
            user= request.user 
            posts = Post.objects.filter(~Q(not_interested = user)).order_by("-da") # inwich user is not inthe list not interested members
    ...
    def post(self, request, format=None):
        # try:
            ...
            data = {'body':body,"user":user.id,'file':file} if file != 'null' else {'body':body,"user":user.id} 
            # data = {'body':body,"user":user.id}
